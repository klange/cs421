...
#define TEST2SARG(WEIGHT, FNAME, ARG1, ARG2)\
    #FNAME^" (parse \""^String.escaped(ARG1)^"\", "^#ARG2^")", mptest WEIGHT (ss_pair1 Solution.FNAME Student.FNAME (parse ARG1, ARG2))
...
#define TEST2SARG_TWOFUN(WEIGHT, FNAME1, FNAME2, ARG1, ARG2)\
    #FNAME1^" (parse \""^String.escaped(ARG1)^"\", "^#ARG2^")", mptest WEIGHT (ss_pair1 FNAME1 FNAME2 (parse ARG1, ARG2))
